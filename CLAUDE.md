...
Tests/
  BuntingTests/         # Unit tests: bucketing, condition evaluation, config store
                        ...
                        # Conformance/ — vendored cross-language test vectors (bucketing,
                        # evaluation, fingerprint, signature, bootstrap) generated by the
                        # admin; ConformanceRunnerTests.swift is the two-axis contract
                        # completion gate. Sync via `cp -R ../bunting-admin/conformance/.
                        # Tests/BuntingTests/Conformance/`.
                        # CLITests.swift exercises the built `bunting-cli` binary against a
                        # BSD-socket LocalFixtureServer on 127.0.0.1 (CI-safe on GitHub
                        # Actions runners; see .github/workflows/swift.yml).
