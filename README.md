...
Create `BuntingConfig.plist` in your app bundle (ensure **Target Membership** is checked). The admin can generate this file for you at `GET /api/bootstrap/plist?appIdentifier=<your-app>`; a platform-neutral JSON equivalent of the same bootstrap document is also available at `/api/bootstrap/json` for non-Apple consumers — Apple apps should keep using the plist:
...
- Condition evaluation (platform, device class, version, region, language, reserved/custom attributes)
...
- **Conformance**: `Tests/BuntingTests/ConformanceRunnerTests.swift` runs a vendored bundle of cross-language test vectors (bucketing, evaluation, fingerprint, signature, bootstrap) generated by the admin from its own reference implementation. Passing this suite is the completion gate for the two-axis targeting contract. Sync the bundle after admin changes with `cp -R ../bunting-admin/conformance/. Tests/BuntingTests/Conformance/`.
- **CLI**: `CLITests.swift` runs the built `bunting-cli` binary against a BSD-socket `LocalFixtureServer` on `127.0.0.1`, so network-touching CLI tests are CI-safe on GitHub Actions runners (`.github/workflows/swift.yml`).
